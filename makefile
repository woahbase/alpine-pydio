...
MOUNTFLAGS := # -v $(CURDIR)/config:/config  -v $(CURDIR)/data:/data # mount in prod
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'nginx -v; php --version; echo "$$(cat /opt/pydio/version)";'
